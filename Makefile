...
test: test.go
	...
	./query cli invoke mychannel test initLedger
	docker exec cli2 peer chaincode install -n test -v $(VERSION) -p myprj
	# instantiate chaincode
	docker exec cli2 peer chaincode instantiate -o orderer.example.com:7050 -n test -v $(VERSION) -C privatechannel -c '{"Args":["init"]}' --tls --cafile /etc/hyperledger/fabric/peer/crypto/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem
	./query cli2 invoke privatechannel test initLedger
